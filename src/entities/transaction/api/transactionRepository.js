...
export const transactionRepository = {
    ...
    console.log("o day oi oi oi ")
      const apiCall = async () => {
        ...
        if (incomeRes.error || transactionRes.error) {
          ...
          // Ném lỗi để kích hoạt retry
          throw incomeRes.error || transactionRes.error; 
        }

        const incomes = incomeRes.data.map((item) => ({ ...item, type: "income" }));
        const transactions = transactionRes.data.map((item) => ({ ...item, type: "transaction" }));
  ...
  async fetchChartData(userId) {
    ...
    const apiCall = async () => {
      ...
      // ... (Giữ nguyên logic xử lý và tính toán dữ liệu)
    ...
    const apiCall = async () => {
      console.log("vao dc fetchhh")
        ...
        if (incomeRes.error || expenseRes.error) {
          ...
          throw incomeRes.error || expenseRes.error;
        }
