...
const calculateNextExecution = (startDate: string, frequency: string): string => {
  ...
  switch (frequency) {
    case "3 phút": // Giữ lại "3 phút" để debug nếu bạn muốn
      date = new Date(); 
...
async function processPeriodicTransactions(supabaseAdmin: any) {
  const nowISO = new Date().toISOString();
  let processedCount = 0;
  let errorCount = 0;

  // 1. Lấy các task định kỳ đã đến hạn
  const { data: periodicList, error } = await supabaseAdmin
    .from("preodic")
    .select("*")
    .lte("next_execution", nowISO) // Task đã đến hạn
  ...
  if (!periodicList || periodicList.length === 0) {
    ...
    return { processed: 0, errors: 0, message: "Không có giao dịch định kỳ nào cần xử lý." };
  }
  ...
  for (const item of periodicList) {
    try {
      ...
      if (new Date(item.endDate) < new Date()) {
        await supabaseAdmin
          .from("preodic")
          .update({ is_active: false }) // Hết hạn, tắt task
          .eq("id", item.id);
      } else {
        await supabaseAdmin
          .from("preodic")
          .update({ next_execution: nextExec }) // Hẹn lịch cho lần tiếp theo
          .eq("id", item.id);
      }
  ...
  return { processed: processedCount, errors: errorCount, message: "Hoàn tất xử lý task định kỳ." };
}
